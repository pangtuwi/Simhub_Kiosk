...
**The keyring-unlock prompt can recur even after wiping and blanking the keyring — the real cause is a misnamed keyring, not a timing issue.** Confirmed on real hardware across several rounds of diagnosis: wiping `~/.local/share/keyrings/*` and leaving the password blank on recreation "fixed" it for one reboot, then the exact same "Authentication required, the login keyring did not get unlocked" prompt came back on a later one. `/etc/pam.d/gdm-autologin` was checked directly and already correctly includes `pam_gnome_keyring.so` in both its `auth` and `session` stacks — ruling out missing PAM wiring. The actual cause: `pam_gnome_keyring.so`'s auto-unlock only ever looks for a keyring literally named **`login`** (`~/.local/share/keyrings/login.keyring`). On this kiosk, the first thing to ever touch the keyring after a wipe was `grdctl` storing RDP credentials — not an interactive login — which made libsecret auto-create its own generically-named default collection (`Default_keyring.keyring`) instead of a `login`-named one. PAM then has nothing to even attempt unlocking: `gkr-pam: no password is available for user` / `couldn't unlock the login keyring` is reporting a *missing target*, not a failed unlock attempt — confirmed directly by inspecting `~/.local/share/keyrings/default` on the affected machine, which named `Default_keyring` rather than `login`.

`setup_rdp.sh` now checks for and repairs this: if the default keyring exists under any name other than `login` (and no `login.keyring` already exists, so it never clobbers one), it renames the file and updates the `default` pointer to `login` before any `grdctl` calls run. Since the currently-running `gnome-keyring-daemon` already has the old file open by inode and won't notice a rename until it restarts, **a reboot after running this script is required** for GDM's PAM stack to actually find the renamed keyring. An earlier attempt at fixing this by running `gnome-keyring-daemon --unlock` from `rdp-reassert.sh` was removed — it didn't address the real cause (the daemon has nothing to unlock, wrong keyring name) and its "another secret service is running" log line suggests it may have been spawning a conflicting second daemon instance rather than helping.
